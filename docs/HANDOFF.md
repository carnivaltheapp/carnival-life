...
- Marker `P3-ROADMAP-OAUTH-REFRESH-154` adds rotating OAuth refresh credentials after production
...
- Marker `P3-ROADMAP-OAUTH-INSPECTOR-151` corrects the consent callback to use HTTP 303 after the
  ...
  both supported scopes. Dynamic registration also permits the strictly validated loopback-IP
  callback used by MCP Inspector while authorization and token exchange retain exact URI matching.
  Marker 151 supersedes the initial 149/150 deployments of the callback fix.
