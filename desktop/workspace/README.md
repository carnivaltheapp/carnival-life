...
path and native-host marker (`DRAWER-HOST-16`). Administrator access is not
...
PlayHouse position invariant: Carnival always anchors the PlayHouse window's
X/Y coordinates to the selected monitor work area's top-left origin. Its width
may be retained or changed, but only Aux participates in horizontal Drawer
animation. Saved or manually moved PlayHouse X/Y coordinates are never treated
as resting geometry.
...
Windows companion with `Carnival native host: DRAWER-HOST-16`; startup is also
