...
test("physical Gmail drop attaches only the row under the pointer and persists replacement", async ({ auth }) => {
  ...
  const { data: googleAccount } = await auth.user
  ...
  await expect(target.getByText("David Example", { exact: true })).toBeVisible();
  const { data: assigned } = await auth.user
    .from("plays")
    .select("player_contact_id, play_type")
    .eq("owner_user_id", auth.userId)
    .eq("title", "Gmail drop target")
    .single();
  expect(assigned?.player_contact_id).toEqual(expect.any(String));
  expect(assigned?.play_type).toBe("normal");
  ...
  await expect.poll(async () => {
    const { data } = await auth.user
      .from("plays")
      .select("player_contact_id, source_metadata")
      .eq("owner_user_id", auth.userId)
      .eq("title", "Gmail drop target")
      .single();
    return data;
    ...
    player_contact_id: assigned?.player_contact_id,
  });
...
test("outside-row region selection skips Appointments and locks row reorder", async ({ auth }) => {
  ...
  expect(error).toBeNull();
  await auth.page.reload();
  await expect(auth.page.getByText("P3-GMAIL-PAYLOAD-TRACE-40", { exact: true })).toBeVisible();
  ...
  await expect(first.getByRole("button", { name: "Flip rank" })).toBeVisible();
  await expect(appointment.getByRole("button", { name: "Flip rank" })).toHaveCount(0);
...
test("selected Plays drag together to a Basket and date in relative order", async ({ auth }) => {
  ...
  const tomorrow = auth.page.getByRole("link", { name: /^Tmrw / });
...
test("Flip rank directly preserves placement and moves the Play to each rank's top", async ({ auth }) => {
  ...
  await createPlay(auth.page, "Flip candidate");
  const { error } = await auth.user.from("plays").insert({
    owner_user_id: auth.userId,
    play_type: "reminder",
    scheduled_date: today,
