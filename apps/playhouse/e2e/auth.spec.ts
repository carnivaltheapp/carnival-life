...
test("unauthenticated screen and build stamp render", async ({ page }) => {
  ...
  await expect(page.getByTestId("version-stamp")).toHaveText("P3-INCOMING-GMAIL-PAYLOAD-SHAPE-104");
});
...
test("disposable authenticated session loads the PlayHouse shell", async ({ auth }) => {
  ...
  await expect(auth.page.getByRole("button", { name: "Show Baskets" })).toBeVisible();
