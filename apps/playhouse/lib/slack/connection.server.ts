...
import { hasRequiredSlackScopes } from "./oauth";
...
export async function resolveSlackNameForOwner(ownerUserId: string, value: string) {
  ...
  if (!hasRequiredSlackScopes(connection.granted_scopes)) {
  ...
  try {
    ...
    return await cache.get(`${connection.slack_team_id}:${resource.type}:${resource.id}`, () =>
      fetchSlackResourceName(token, resource, connection.slack_user_id));
  } catch (error) {
